        ...
        <tr {if $position_identifier} id="tr_{$position_group_identifier}_{$tr.$identifier}_{if isset($tr.position['position'])}{$tr.position['position']}{else}0{/if}"{/if} class="{if isset($tr.class)}{$tr.class}{/if} {if $tr@iteration is odd by 1}odd{/if}"{if isset($tr.color) && $color_on_bg} style="background-color: {$tr.color}"{/if} >
            ...
            {foreach $fields_display AS $key => $params}
                ...
                {/if}
                    ...
                    {elseif isset($params.activeVisu)}
                        {if $tr.$key}
                            <i class="icon-check-ok"></i>
                                {l s='Enabled' mod='tvcmsproductcomments'}
                        {else}
                            <i class="icon-remove"></i>
                            {l s='Disabled' mod='tvcmsproductcomments'}
                        {/if}
                    ...
                    {elseif isset($params.type) && $params.type == 'percent'}
                        {$tr.$key} {l s='%' mod='tvcmsproductcomments'}
                    {elseif isset($params.type) && $params.type == 'bool'}
                        {if $tr.$key == 1}
                            {l s='Yes' mod='tvcmsproductcomments'}
                        {elseif $tr.$key == 0 && $tr.$key != ''}
                            {l s='No' mod='tvcmsproductcomments'}
                        {/if}
            ...
            {if $has_actions}
                <td class="text-right">
                    ...
                    {/if}
                    {if !empty(Module::getInstanceByName('reviewsreply')) && isset(Module::getInstanceByName('reviewsreply')->active)}
                        {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_tvcmsproduct_comment'}
                            <div class="pull-right">
                                <a onclick="$(this).closest('tr').next('tr').show();" class="btn btn-small btn-default"><i class="icon-pencil"></i> {l s='Reply' mod='tvcmsproductcomments'}</a>
                            </div>
                        {/if}
                    {/if}
                </td>
            ...
            {if Module::getInstanceByName('reviewsreply')->active == true && $identifier == 'id_tvcmsproduct_comment'}
                <tr style="display:none;">
                    <td colspan="8" class="center" style="padding:8px">
                        {assign var='reply' value=rreply::getByIdTvcmsProductCommentAll($tr.id_tvcmsproduct_comment)}
                        <form method="post">
                            <input type="hidden" name="action" value="addreply"/>
                            <input type="hidden" name="id_tvcmsproduct_comment" value="{$tr.id_tvcmsproduct_comment}"/>
                            {if isset($reply.0)}
                            ...
                            <div class="col-lg-12">
                                <button class="btn btn-large btn-success">
                                    <i class="icon-save"></i> {if isset($reply.0)}{l s='Save reply' mod='tvcmsproductcomments'}{else}{l s='Add reply' mod='tvcmsproductcomments'}{/if}
                                </button>
                            </div>
                        </form>
                    </td>
    ...
    <tr>
        <td class="list-empty" colspan="{count($fields_display)+1}">
            <div class="list-empty-msg">
                ...
                {l s='No records found' mod='tvcmsproductcomments'}
            </div>
        </td>
    </tr>
{/if}
