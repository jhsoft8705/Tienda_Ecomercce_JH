...
class TvcmsCustomSettingDbUpgrade
{
    ...
    public function DBUpgrade360($pthis)
    {
        ...
        try {
            $languages = Language::getLanguages();
            for ($i = 1; $i <= 4; ++$i) {
                $ImageSizePath = _PS_MODULE_DIR_ . 'tvcmsmultibanner2/views/img/';
                $result = [];
                foreach ($languages as $lang) {
                    $ImageName = Configuration::get('TVCMSMULTIBANNER2_IMAGE_NAME_' . $i, $lang['id_lang']);
                    if (file_exists($ImageSizePath . $ImageName)) {
                        $imagedata = getimagesize($ImageSizePath . $ImageName);
                        $width = $imagedata[0];
                        $height = $imagedata[1];
                ...
                $width = $result['TVCMSMULTIBANNER2_IMAGE_WIDTH_' . $i];
                Configuration::updateValue('TVCMSMULTIBANNER2_IMAGE_WIDTH_' . $i, $width);
                $height = $result['TVCMSMULTIBANNER2_IMAGE_HEIGHT_' . $i];
                Configuration::updateValue('TVCMSMULTIBANNER2_IMAGE_HEIGHT_' . $i, $height);
            }
        } catch (Exception $e) {
            if ($this->TVDEBUG_DB) {
                $this->message .= $pthis->displayError($pthis->l('TVDEBUG_DB : TVCMSMULTIBANNER2_IMAGE_NAME Configuration issue.'));
            }
        }
