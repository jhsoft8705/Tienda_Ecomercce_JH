								...
								{elseif $input.type == 'switch'}
									<span class="switch prestashop-switch fixed-width-lg">
										...
										<label {if $value.value == 1} for="{$input.name}_on"{else} for="{$input.name}_off"{/if}>
											{if $value.value == 1}
												{l s='Yes' mod='tvcmsblog'}
											{else}
												{l s='No' mod='tvcmsblog'}
											{/if}
										</label>
										{/strip}										{/foreach}
								...
								{elseif $input.type == 'change-password'}
									<div class="row">
										<div class="col-lg-12">
											<button type="button" id="{$input.name}-btn-change" class="btn btn-default">
												<i class='material-icons'>&#xe897;</i>
												{l s='Change password...' mod='tvcmsblog'}
											</button>
											<div id="{$input.name}-change-container" class="form-password-change well hide">
												<div class="form-group">
													<label for="old_passwd" class="control-label col-lg-2 required">
														{l s='Current password' mod='tvcmsblog'}
													</label>
													<div class="col-lg-10">
												...
												<div class="form-group">
													<label for="{$input.name}" class="required control-label col-lg-2">
														<span class="label-tooltip" data-toggle="tooltip" data-html="true" title="" data-original-title="Password should be at least 8 characters long.">						
															{l s='New password' mod='tvcmsblog'}
														</span>
													</label>
													<div class="col-lg-9">
														<div class="input-group fixed-width-lg">
												...
												<div class="form-group">
													<label for="{$input.name}2" class="required control-label col-lg-2">
														{l s='Confirm password' mod='tvcmsblog'}
													</label>
													<div class="col-lg-4">
														<div class="input-group fixed-width-lg">
															<span class="input-group-addon">
												...
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														...
														<button type="button" id="{$input.name}-generate-btn" class="btn btn-default">
															...
															{l s='Generate password' mod='tvcmsblog'}
														</button>
													</div>
												</div>
												<div class="form-group">
													<div class="col-lg-10 col-lg-offset-2">
														<p class="checkbox">
															<label for="{$input.name}-checkbox-mail">
																...
																{l s='Send me this new password by Email' mod='tvcmsblog'}
															</label>
														</p>
													</div>
												</div>
									...
									<script>
										$(function(){
											...
											var feedback = [
												{ badge: 'text-danger', text: '{l s='Invalid' mod='tvcmsblog' js=1}' },
												{ badge: 'text-warning', text: '{l s='Okay' mod='tvcmsblog' js=1}' },
												{ badge: 'text-success', text: '{l s='Good' mod='tvcmsblog' js=1}' },
												{ badge: 'text-success', text: '{l s='Fabulous' mod='tvcmsblog' js=1}' }
											];
											...
											}, '{l s='Invalid password confirmation' mod='tvcmsblog' js=1}');
										...
										<select name="{$key}" class="fixed-width-lg{if isset($input.class)} {$input.class}{/if}">
											...
											{if $key == 'months'}
												{*
													...
													{l s='January' mod='tvcmsblog'}
													{l s='February' mod='tvcmsblog'}
													{l s='March' mod='tvcmsblog'}
													{l s='April' mod='tvcmsblog'}
													{l s='May' mod='tvcmsblog'}
													{l s='June' mod='tvcmsblog'}
													{l s='July' mod='tvcmsblog'}
													{l s='August' mod='tvcmsblog'}
													{l s='September' mod='tvcmsblog'}
													{l s='October' mod='tvcmsblog'}
													{l s='November' mod='tvcmsblog'}
													{l s='December' mod='tvcmsblog'}
												*}
												{foreach $select as $k => $v}
													<option value="{$k}" {if $k == $fields_value[$key]}selected="selected"{/if}>{l s=$v mod='tvcmsblog'}</option>
												{/foreach}
											{else}
												{foreach $select as $v}
				...
				{if isset($fieldset['form']['submit']) || isset($fieldset['form']['buttons'])}
					<div class="panel-footer">
						...
						<a href="{$back_url|escape:'html':'UTF-8'}" class="btn btn-default" onclick="window.history.back();">
							<i class="process-icon-cancel"></i> {l s='Cancel' mod='tvcmsblog'}
						</a>
						{/if}
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('.datetimepicker').datetimepicker({
				...
				currentText: '{l s='Now' mod='tvcmsblog'}',
				closeText: '{l s='Done' mod='tvcmsblog'}',
				ampm: false,
				amNames: ['AM', 'A'],
				pmNames: ['PM', 'P'],
				timeFormat: 'hh:mm:ss tt',
				timeSuffix: '',
				timeOnlyTitle: '{l s='Choose Time' mod='tvcmsblog' js=1}',
				timeText: '{l s='Time' mod='tvcmsblog' js=1}',
				hourText: '{l s='Hour' mod='tvcmsblog' js=1}',
				minuteText: '{l s='Minute' mod='tvcmsblog' js=1}',
			});
...
<div class="panel" id="fieldset_generate_img">
	<div class="panel-heading">
		<i class="icon-AdminParentPreferences"></i>  {l s='Regenerate Image Thumbnail' mod='tvcmsblog'}
	</div>
	<div class="form-wrapper">
		<div class="form-group">
			<label class="control-label col-lg-3">
				{l s='Select Image Type' mod='tvcmsblog'}
			</label>
			<div class="col-lg-9">
				<select name="select_image_type" class=" fixed-width-xl" id="select_image_type">
					<option value="all">{l s='ALL Blog Image' mod='tvcmsblog'}</option>
					<option value="post">{l s='All Blog Post Image' mod='tvcmsblog'}</option>
					<option value="gallery">{l s='All Blog Post Gallery Image' mod='tvcmsblog'}</option>
					<option value="category">{l s='All Blog Category Image' mod='tvcmsblog'}</option>
				</select>
				<p class="help-block">
					{l s='Please Select Which Type of Image You Want To Regenerate.' mod='tvcmsblog'}
				</p>
			</div>
		</div>
	</div>
	<div class="panel-footer">
		<button type="submit" value="1" id="submit_generateimage" name="submit_generateimage" class="btn btn-default pull-right">
			<i class="process-icon-save"></i> {l s='Save' mod='tvcmsblog'}
		</button>
