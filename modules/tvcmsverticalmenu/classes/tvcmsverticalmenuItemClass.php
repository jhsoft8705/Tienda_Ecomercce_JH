...
*/
class TvcmsVerticalMenuItemClass extends ObjectModel
{
    public $id_column;
    ...
    public $temp_url = '{lab_menu_h_url}';

    public static $definition = [
        'table' => 'tvcmsverticalmenu_item',
        'primary' => 'id_item',
        'multilang' => true,
        'multilang_shop' => true,
        'fields' => [
            ...
            'text' => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'],
            'position' => [
                'type' => self::TYPE_INT,
                'shop' => true,
                'validate' => 'isunsignedInt',
    ...
    public function __construct($id_item = null, $id_lang = null, $id_shop = null, Context $context = null)
    {
        ...
        Shop::addTableAssociation('tvcmsverticalmenu_item', ['type' => 'shop']);
        Shop::addTableAssociation('tvcmsverticalmenu_item_lang', ['type' => 'fk_shop']);
        if ($this->id) {
            ...
            foreach ($languages as $language) {
                if (isset($this->text[(int) $language['id_lang']])
                    ...
                    $temp = str_replace($this->temp_url, _PS_BASE_URL_
                        . __PS_BASE_URI__, $this->text[(int) $language['id_lang']]);
    ...
    public function getFieldShop($field)
    {
        ...
        $sql = 'SELECT wms.' . $field . ' FROM ' . _DB_PREFIX_ . 'tvcmsverticalmenu_item wm
        LEFT JOIN ' . _DB_PREFIX_ . 'tvcmsverticalmenu_item_shop wms ON (wm.id_item = wms.id_item)
    ...
    public function uploadImage($feild, $folder)
    {
        ...
        if (isset($_FILES[$feild]) &&
            ...
            !empty($imagesize) &&
            in_array(Tools::strtolower(
                Tools::substr(strrchr($imagesize['mime'], '/'), 1)
            ), ['jpg', 'gif', 'jpeg', 'png']) &&
