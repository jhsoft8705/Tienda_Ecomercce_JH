...
include_once 'tvcmsverticalmenu.php';

$home_menu = new TvcmsVerticalMenu();
$menus = [];
...
if (Tools::getValue('action') == 'updateMenusPosition' && Tools::getValue('menu')) {
    ...
    foreach ($menus as $position => $id_menu) {
        Db::getInstance()->execute('
            UPDATE `' . _DB_PREFIX_ . 'tvcmsverticalmenu_shop` SET `position` = ' . (int) $position
            . ' WHERE `id_tvcmsverticalmenu` = ' . (int) $id_menu . ' AND id_shop = ' . $id_shop);
    }
    $home_menu->clearCustomSmartyCache('tvcmsverticalmenu.tpl');
}
if (Tools::getValue('action') == 'updateMenusItemPosition' && Tools::getValue('menuitem')) {
    ...
    foreach ($menu_items as $position => $id_menuitem) {
        Db::getInstance()->execute('
            UPDATE `' . _DB_PREFIX_ . 'tvcmsverticalmenu_item_shop` SET `position` = ' . (int) $position
...
if (Tools::getValue('action') == 'updateColumnsPosition' && Tools::getValue('col')) {
    ...
    foreach ($col_items as $position => $id_col) {
        Db::getInstance()->execute('
            UPDATE `' . _DB_PREFIX_ . 'tvcmsverticalmenu_column_shop` SET `position` = ' . (int) $position
