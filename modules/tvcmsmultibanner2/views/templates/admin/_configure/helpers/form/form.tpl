...
{if $input.type == 'BtnInstallData'}
        ...
        <button class="btn btn-primary tvcms-sample-install-btn" type="submit" onclick="return confirmAction()" name="submitTvcmsSampleinstall">{l s='Install Sample Data' mod="tvcmsmultibanner2"}</button>
{/if}
                        ...
                        <div class="dummyfile input-group">
                            ...
                            <span class="input-group-btn">
                                <button id="{$input.name|escape:'htmlall':'UTF-8'}_{$language.id_lang|escape:'htmlall':'UTF-8'}-selectbutton" type="button" name="submitAddAttachments" class="btn btn-default">
                                    <i class="icon-folder-open"></i> {l s='Choose a file' mod='tvcmsmultibanner2'}
                                </button>
                            </span>
                        </div>
                    </div>
    ...
    {if $input.type == 'multiple_select'}
        <div class="col-lg-9">
            {foreach from=$languages item=language}
                ...
                <div class="form-group">
                    <div class="col-lg-3">
                        ...
                        <select name="{$input.name|escape:'htmlall':'UTF-8'}_{$language.id_lang|escape:'htmlall':'UTF-8'}">
                            <option value='left' {if $value == 'left'}selected="selected"{/if}>{l s='Left' mod='tvcmsmultibanner2'}</option>
                            <option value='top-left' {if $value == 'top-left'}selected="selected"{/if}>{l s='Top Left' mod='tvcmsmultibanner2'}</option>
                            <option value='bottom-left' {if $value == 'bottom-left'}selected="selected"{/if}>{l s='Bottom Left' mod='tvcmsmultibanner2'}</option>
                            <option value='center' {if $value == 'center'}selected="selected"{/if}>{l s='Center' mod='tvcmsmultibanner2'}</option>
                            <option value='top-center' {if $value == 'top-center'}selected="selected"{/if}>{l s='Top Center' mod='tvcmsmultibanner2'}</option>
                            <option value='bottom-center' {if $value == 'bottom-center'}selected="selected"{/if}>{l s='Bottom Center' mod='tvcmsmultibanner2'}</option>
                            <option value='right' {if $value == 'right'}selected="selected"{/if}>{l s='Right' mod='tvcmsmultibanner2'}</option>
                            <option value='top-right' {if $value == 'top-right'}selected="selected"{/if}>{l s='Top Right' mod='tvcmsmultibanner2'}</option>
                            <option value='bottom-right' {if $value == 'bottom-right'}selected="selected"{/if}>{l s='Bottom Right' mod='tvcmsmultibanner2'}</option>
                            <option value='none' {if $value == 'none'}selected="selected"{/if}>{l s='None' mod='tvcmsmultibanner2'}</option>
                        </select>
                    </div>
                ...
                <div class="form-group">
                    <div id="{$input.name|escape:'htmlall':'UTF-8'}-{$language.id_lang|escape:'htmlall':'UTF-8'|escape:'htmlall':'UTF-8'}-images-thumbnails" class="col-lg-12">
                        <p class="help-block">{l s='Select Caption Side' mod='tvcmsmultibanner2'}</p>
                    </div>
                </div>
